r"""The nonlinear n*-warp  vs the harmonic comb: the 2^s - 1 half-shift payoff (#119).

Phase 2 of the discrete <-> continuous bridge (epic #116). Phase 1 (harmonic_bridge.py,
#118) restored discreteness *additively* -- it added Fourier harmonics of the
discreteness comb to the integrand and watched the zeros migrate. Here we do #116's
...
of 1 - x on [0,1], #116's kernel). As K -> infinity, phi_K(x) -> 1/2 - {x}, so
x + phi_K(x) -> floor(x) + 1/2: the warp collapses each unit cell [m, m+1] onto its
**midpoint m + 1/2**. That half-integer pinning is the whole story (sec 4 of the
write-up): it is why the warp's natural target is a *half-shifted* zeta.
...
-------------------------------------------
  * K = 0 (phi_0 = 0): warp_K(s, 0) = int_0^1 zeta(s, 1+u) du = int_1^inf x^{-s} dx =
    1/(s-1) -- exactly the bland zeta-side endpoint of #116. (`warp_K(.,0)` is *exact*.)
...
exactly the #118 lesson in warp form. Without it the limit is zeta(s, 3/2) and the
sigma=0 companion below is *absent* (e.g. |zeta(s,3/2)| = 1 at s = 2 pi i/ln2, where
|zeta(s,1/2)| = 0). With it, the (2^s - 1) factor that #116 set aside re-emerges *on
...
#118, imported from harmonic_bridge):
  ...
  * a companion family, the zeros of the prefactor 2^s - 1 at t = 2 pi k/ln2, migrates
    ...
    equation, of #118's eta companion on sigma = 1, at the *same* heights. (`PREF_HEIGHTS`.)
...
Math write-up: ../knowledge/sum-integral/nstar-warp-half-shift.md (Phase 2, #119/#123).
Run directly to validate + plot: writes sum_integral/figures/warp_bridge.png.
"""
import math
import sys
...
import cont_eta as ce         # noqa: E402  (Phase 0: the #117 Riemann-von Mangoldt counting_law)
...
#
# High-tau stability (#130). The moment series has three |Im s|-dependent failure modes that
...
_GUARD_ONSET = 40.0   # |Im s| below which the base dps / Jmax=60 / degree=8 already converge (#130)
...
def _moment_dps(s):
    ...
    to absorb the binomial-moment cancellation (#130; see the block comment above)."""
...
def _moment_jmax(s):
    ...
    count must grow with |Im s| or a fixed cap truncates it before it converges (#130)."""
    tau = float(abs(mp.im(s)))
...
def _gl_degree(s):
    ...
    (#130). Degree 8 (~384 nodes) holds to |Im s|~150; each +1 doubles the nodes."""
    tau = float(abs(mp.im(s)))
...
def _moment_series(s, logs, mus, degree, base_dps):
    ...
    *ambient* accuracy base_dps (not the inflated working precision): that keeps the
    ...
    amplifies floored-tail garbage (#130). Shared by warp_K and warp_alpha.warp_alpha.
    """
...
def _warp_grid(K, degree, Jmax=60):
    ...
    up to Jmax, rebuilding with more terms when a higher |Im s| needs a larger Jmax (#130)."""
...
def warp_K(s, K):
    """warp_K(s, K) = int_1^inf (x + phi_K)^{-s} dx (sec 2c), the fast grid+moment evaluator.

    K = 0 gives 1/(s-1) exactly; K -> infinity gives zeta(s, 3/2) (the cell midpoints
    3/2, 5/2, ... -- the *honest* int_1^inf limit). Valid for all sigma, and (#130) for all
...
# below #118's 200 only because the picture is fully settled by then.
K_SCHEDULE = [1, 2, 3, 4, 5, 7, 9, 12, 16, 21, 27, 35, 45]


# --------------------------------------------------------------------------
# the sigma=1/2 U sigma=0 density bijection -- MEASURED (issue #123)
# --------------------------------------------------------------------------
# #118 *counted* the eta split (harmonic_bridge._print_counting): #eta-zeros(<T) = the
# sigma=1/2 zeta zeros + the sigma=1 prefactor zeros = #117's Riemann-von Mangoldt N(T).
...
# functional equation"). The densities sum exactly to #117's ground density
#   (1/2pi)ln(t/2pi) [zeta, sigma=1/2] + (ln2/2pi) [companion, sigma=0] = (1/2pi)ln(t/pi),
# so N_1/2(T) + N_0(T) reproduces #117's RvM total to leading order. (The integrated counts
# carry an O(1) offset of 1/4: zeta's RvM constant is 7/8, #117's counting_law has 5/8 -- a
# constant, not a density, mismatch; the eta side of #118 has the identical 1/4 offset.)
def target_half(s):
...
def _print_counting(T=40.0):
    """Witness the warp density bijection by MEASUREMENT (#123) -- the mirror of #118's eta count.
    ...
    against #117's Riemann-von Mangoldt counting_law -- the same total the eta split reproduced
    on sigma=1/2 U sigma=1.
    """
    counts, off, zeros = count_on_lines(target_half, T)
    nhalf, n0 = counts[0.5], counts[0.0]
    rvm = float(ce.counting_law(T))                       # (T/2pi)ln(T/pi e) + 5/8 (#117)
    ...
    print(f"   N_1/2 + N_0 = {nhalf} + {n0} = {nhalf + n0}   vs   #117 RvM N(T) = {rvm:.3f}   "
...
def _main():
    ...
    cz = _print_migration("comb zeta_K (#118): zeta zeros born high, descend onto sigma=1/2",
                          zeta_targets, fn=hb.zeta_K, schedule=K_SCHEDULE)
    _print_counting()      # the measured sigma=1/2 U sigma=0 density bijection (#123)
    ...
    ax[0].plot([], [], "--s", color="C1", mfc="none", label="comb #118 (descends)")
    ...
    ax[1].axvline(1.0, ls="--", lw=1, color="C1", label=r"$\sigma=1$ ($\eta$ mirror, #118)")
    ...
    fig.suptitle(r"$n^*$-warp vs harmonic comb: the half-shifted $\zeta(s,1/2)=(2^s-1)\zeta(s)$ "
                 r"and its $\sigma=0$ companion  (issue #119)", fontsize=12)
