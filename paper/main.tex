...
$\mathbb{C}$, with no abscissa of convergence. \Cref{sec:two} reads the
...
\item \textbf{The midpoint phase $\alpha=\tfrac12$ is annihilated.}
  ...
  is \emph{exactly the zero set of the $\eta$ sign-carrier} $\cos(\pi x)$ --- so
...
keeping the pristine zeroless endpoint \emph{and} a clean line --- precisely
because its integrand $x^{-s}$ has no zeros on the half-integer lattice. The $\eta$
...
\begin{figure}[t]
  ...
  are exactly the zeros of the $\eta$ sign-carrier $\cos(\pi x)$, while the integers
...
the birth-$K$ law --- higher, denser zeros need proportionally more harmonics ---
and it depends on $t$, not on any vertical line in $\sigma$.
...
there is no divergence at any $K$ --- more harmonics always help.
...
\begin{figure}[t]
  ...
  for every $\sigma$, including $\sigma<0$ where both Dirichlet series diverge ---
...
created --- the same fixed-cardinality, density-preserving motion of a pre-existing
...
is \emph{sorted} onto two lines with the exact bookkeeping of
